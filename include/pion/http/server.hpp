...
public:
    ...
    /// default destructor
    virtual ~server() { if (isListening()) stop(); }
    ...
    explicit server(const unsigned int tcp_port = 0)
        : tcp::server(tcp_port),
        m_bad_request_handler(server::handleBadRequest),
        m_not_found_handler(server::handleNotFoundRequest),
        m_server_error_handler(server::handleServerError),
        m_max_content_length(http::parser::DEFAULT_CONTENT_MAX)
    { 
        setLogger(PION_GET_LOGGER("pion.http.server"));
    }
     ...
     */
    void addResource(const std::string& resource, request_handler_t request_handler);
    ...
    inline void setBadrequest_handler_t(request_handler_t h) { m_bad_request_handler = h; }

    /// sets the function that handles requests which match no other web services
    inline void setNotFoundHandler(request_handler_t h) { m_not_found_handler = h; }

    /// sets the function that handles requests which match no other web services
    inline void seterror_handler_t(error_handler_t h) { m_server_error_handler = h; }
     ...
     */
    static void handleBadRequest(http::request_ptr& http_request_ptr,
     ...
     * @param tcp_conn the TCP connection that has the new request
     */
    static void handleNotFoundRequest(http::request_ptr& http_request_ptr,
                                      tcp::connection_ptr& tcp_conn);
     ...
     */
    static void handleServerError(http::request_ptr& http_request_ptr,
                                  tcp::connection_ptr& tcp_conn,
     ...
     * @param error_msg message that explains what went wrong
     */
    static void handleForbiddenRequest(http::request_ptr& http_request_ptr,
                                       tcp::connection_ptr& tcp_conn,
                                       const std::string& error_msg);
     ...
     */
    static void handleMethodNotAllowed(http::request_ptr& http_request_ptr,
                                       tcp::connection_ptr& tcp_conn,
...
protected:
     ...
     */
    virtual void handleRequest(http::request_ptr& http_request_ptr,
     ...
     */
    virtual bool findrequest_handler_t(const std::string& resource,
