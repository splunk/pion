...
public:
    ...
    typedef std::function<void(http::response_ptr&, tcp::connection_ptr&, const asio::error_code&)>   finished_handler_t;
...
protected:
    ...
    virtual void read_bytes(void) {
        get_connection()->async_read_some(std::bind(static_cast<void(response_reader::*)(const asio::error_code&, std::size_t)>(&response_reader::consume_bytes),
                                                        shared_from_this(),
                                                        std::placeholders::_1,
                                                        std::placeholders::_2));
    }
