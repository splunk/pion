...
// ReportService member functions

/// handles requests for ReportService
void ReportService::operator()(HTTPRequestPtr& request, TCPConnectionPtr& tcp_conn)
{
	// this web service uses static text to test the mixture of "copied" with
	// "static" (no-copy) text
	static const std::string REQUEST_REPORT_TEXT("[Request Report]");
	...
	writer->writeNoCopy(REQUEST_REPORT_TEXT);
...
/// creates new ReportService objects
extern "C" PION_SERVICE_API pion::plugins::ReportService *pion_create_ReportService(void)
{
	return new pion::plugins::ReportService();
}

/// destroys ReportService objects
extern "C" PION_SERVICE_API void pion_destroy_ReportService(pion::plugins::ReportService *service_ptr)
{
	delete service_ptr;
}
