...
public:
    ...
    static inline boost::shared_ptr<HTTPRequestReader>
        create(tcp::connection_ptr& tcp_conn, FinishedHandler handler)
    {
...
protected:
     ...
     * @param handler function called after the message has been parsed
     */
    HTTPRequestReader(tcp::connection_ptr& tcp_conn, FinishedHandler handler)
        : HTTPReader(true, tcp_conn), m_http_msg(new HTTPRequest),
        m_finished(handler)
    {
        m_http_msg->setRemoteIp(tcp_conn->get_remote_ip());
        setLogger(PION_GET_LOGGER("pion.http.HTTPRequestReader"));
    }
        
    /// Reads more bytes from the TCP connection
    virtual void readBytes(void) {
        get_connection()->async_read_some(boost::bind(&HTTPRequestReader::consumeBytes,
                                                        shared_from_this(),
